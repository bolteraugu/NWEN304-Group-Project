...
async function initialRenderingRecipeDetails() {
  if (window.localStorage.getItem("token") != null && window.localStorage.getItem("token").length !== 0) {
    let hasRecipe = false;
    await fetch('http://localhost:8080/cookbook/' + cookbookID + '/checkRecipe/' + recipeID, {
      headers: {
...
async function addListeners() {
  if(document.getElementById("removeRecipe").style.display != "none") {
    document.getElementById("removeRecipe").addEventListener('click', () => removeRecipe(cookbookID, recipeID));
  }
  else {
    document.getElementById("saveRecipe").addEventListener('click', () => addRecipe(cookbookID, recipeID));
  }
}
